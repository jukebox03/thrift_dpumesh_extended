...
이 보고서는 **시간 순서가 아니라 결론 순서**로 읽도록 구성했다. §0이 전체 결론이고, §2~§8이 각
결론을 raw data로 뒷받침한다. 동일 실험을 리팩터 전후로 두 번 측정한 경우, 두 dataset을 같은
결론 아래 나란히 두었다(원본 측정 + 리팩터 후 재현). §9는 그 리팩터의 코드 변경 내역이다.

> **이 문서를 읽는 법.** 각 수치에 근거 등급을 표시한다. **[측정]** = 데이터로 직접 측정(CSV·sweep
> 로그). **[산술]** = 측정값에 산술(예: dma_copy/s = RPS×4). **[귀인]** = 측정값을 소거법/모델로 해석한
> 것 — *직접 증거가 아니며 틀릴 수 있다*(§7.2 참고). **[미측정]** = 직접 증거 없음. raw data는
> 신뢰할 수 있으나, "병목의 원인"류 귀인은 여러 번 번복되었으므로(§7.2) 등급을 보고 받아들일 것.
...
2. **단일 EU chain은 누적 최적화로 sustainable 52K→74K RPS, overload 53.5K→77.4K RPS** [측정], 전
   ...
   pure single-EU engine(556K)의 55.7% (§5). comch 메시지층 리팩터 후에도 74K를 회귀 없이 재현(§3.4).
...
4. **engine은 chain보다 훨씬 빠르다** [측정]: pure single-EU 556K dma_copy/s(size 무관 → op-rate
   ...
   M2(drain+forward) N≥4 ~1.0M plateau (§5). chain 2-EU 416K dma_copy/s [산술] = M2-N=2(625K)의 67%.
5. **chain 천장은 in-flight 깊이가 아니다** [측정]: depth 2048→4096에서 throughput 평탄(~103K),
   latency만 깊이에 비례, 더 밀면 achieved 하락 — 교과서적 M/M/1 고정-서비스율 큐(§6.1의 가장 강한
   positive 증거).
6. **DPU ARM 제어평면을 2 코어로 분할해도 throughput 무변화** [측정]: sends-only −0.9%, rebalanced
   +2%→0%(노이즈). 분할은 0-fail로 correctness만 입증(§6.3).
7. **comch 메시지층 리팩터(enum 통일·완료 메시지 28→16B·TX_ACK 12→8B·vestigial 핸드셰이크 제거·
   안전수정 5종)는 비회귀** [측정]: §3.2(74K)·§4.1(N=2≈N=4≈104K)·§6.3(split 처리율 중립)이 모두
   재현되고 전 구간 0-fail·slot leak 없음 (§9).

### 병목의 원인 — [귀인], 직접 확증되지 않음

- **chain이 416K(2-EU)/309K(1-EU)에서 멈추는 *원인*은 직접 측정되지 않았다.** §6.1의 M/M/1 거동은
  측정되었으나, 그 고정-서비스율을 "2개 활성 EU의 결합된 per-RTT 작업"으로 *국소화*한 것은 소거법에
  의한 귀인이다(§7.2). 결정적 실험(독립 multi-ARM 샤딩 / >2 pod로 활성 EU 증가)은 **아직 안 했다**.
- **pure N=8 regression의 원인 "공유 DMA-engine op-rate 천장 ~1.6M"** 역시 소거법뿐이다(spin/
  ...
  positive 증거는 없음 — occupancy는 polling으로 무용)(§7.2).
...
## 2. 결론 1 — dpumesh transport는 TCP/Envoy 대비 latency 2~3 자릿수 우위 [측정]

(rps=40000, dur=10s, size=8192B)
...
## 3. 결론 2 — 단일 EU chain은 74K RPS까지 최적화되고, 리팩터 후에도 회귀 없이 재현된다 (8KB, 0 fail 전 구간)
...
### 3.2 누적 최적화 경로 (각 단계 = 측정 endpoint) [측정]
...
### 3.3 누적 최적화의 robust 결론 + per-step 귀인의 한계 (정직성 주석)
...
- 그러나 **개별 기법의 기여 배분은 run-to-run 노이즈(~±1.3%)에 묻히는 경우가 있다.** feature-strip
  단계 안의 +1.7% 같은 sub-step delta는 §5/§6의 노이즈 바닥과 같은 크기다. 큰 단계(fence/batch
...
### 3.4 리팩터 후 재현 — §3.2 회귀 없음 [측정]

comch 메시지층 리팩터(§9) 직후 단일 EU(N=1, split off)를 재측정. 측정 환경 §1.5 동일
(BlueField-3, 8KB, dur=10s, fair-pin, EU affinity ON, `-l 40`).
...
처리율 중립(host control send는 chain 병목이 아니므로 — §6.3과 정합).

---

## 4. 결론 3 — chain throughput은 활성 EU 수에 스케일한다 (76K→104K)

### 4.1 chain N-스케일링 [측정]

ring→EU는 `pod%N` 할당이므로(코드 확증, `dpa.c:1084`), 2-pod echo는 N≥2에서 항상 활성 EU가 2개다.
따라서 N=2와 N=4가 같은 천장을 보인다.
...
- fair(1-core host) ≈ hw(2-core host) ≈ 104K → **병목은 host posting 아님** [측정] (§4.3).
- N=1 ~76K = §3.2 fence/batch(77,434) 재현(리팩터 회귀 0).
- **multi-EU 실이득**: 2 활성 EU가 76K→104K(+37%, 1.37×). pure 2-EU 1.93×보다 낮음 = chain 추가 작업.

### 4.2 리팩터 후 N=2 / N=4 재현 — §4.1 회귀 없음 [측정]

comch 리팩터(§9) 후 DPA multithread 재측정(split off). knob `DPUMESH_DPA_THREADS`로 EU 수 주입.
...
— 2-pod은 `pod%N`으로 활성 EU 2개 고정(§4.1 코드 확증 재현). §4.1 기록(N=2 105K→104,100, N=4→104,185)과
일치.

### 4.3 fair ≈ hw — host posting/core 수는 병목이 아니다 [측정]

§4.1 표에서 fair(1-core host)와 hw(2-core host)가 전 target에서 ≈104K로 일치한다(105K: 104,185 vs
104,220). host CPU를 2배로 줘도 chain 천장이 안 움직임 → **chain cap은 host posting/core 수가 아니다.**
(이 사실은 §6.4의 "부정된 가설"에도 정리.)

---

## 5. 결론 4 — engine은 chain보다 훨씬 빠르다: 측정된 천장 계층 [측정]
...
DPU completion이 섞인 **측정 하한**(§5.5, §7.3) — 진짜 multi-EU engine 천장은 더 높을 수 있다.

### 5.3 N=8 regression — 직접 배제한 후보 (2026-06-02, run_pure_dma.sh) [측정]
...
약간 악화). **단일 drain 배제**: M0가 같은 단일 consumer_pe로 N=8 1.53M 도달(§5.4).
...
### 5.5 pure/M0/M2/chain 같은-EU-수 비교 [측정+산술]
...
416K = M2-N=2(625K)의 **67%** (여유 1.5×). 나머지 33% gap의 *원인*은 §7.2 참조(직접 측정 안 됨).

---

## 6. 결론 5 — chain 천장이 *아닌* 것들 (직접 배제)

chain의 cap은 HW도 EU 수도 host도 in-flight 깊이도 ARM compute도 아니다. 각각을 직접 배제한 측정:

### 6.1 in-flight 깊이 아님 — M/M/1 고정 서비스율 (가장 강한 positive 증거) [측정]
...
### 6.3 ARM compute/split 아님 — DPU ARM 멀티코어 분할 throughput 무효 [측정]
...
**리팩터 후 split 재현 [측정]** — 2-EU(N=2) 위 ARM 기능분할 재측정. off baseline = §4.2 N=2
(105K→104,202, p99 20.58ms).
...
**split(1/2)는 처리율 천장을 올리지 못한다**(§6.3 off 결론 재확인). raw 관찰: 이 run에서 rebalanced는
elbow latency가 악화(105K p99 170.97ms vs off 20.58ms·sends-only 25.44ms)되고 105K가 이미 overload —
*원인은 미측정*(단일 run, run-to-run 노이즈 ±1.3% 및 rebalanced 경로 추가 가능성 둘 다 배제 못 함).

### 6.4 host posting/core 수 아님 [측정]

§4.3: fair(1-core) ≈ hw(2-core) ≈ 104K → host CPU 예산은 chain 천장을 안 움직인다.

### 6.5 직접 부정된 가설 (요약) [측정]

- ❌ PCIe BW contention (§5.3 ⑤ + 별도 direction-balance −0.3%)
...
- ❌ desc 32B pack로 PCIe read 절감 (line-granular writeback이 이웃 clobber, §3.3)
- ❌ Method 0/2 = HW max (host descriptor feed-bound였음, pure 556K가 입증)
- ❌ in-flight 깊이가 chain cap (§6.1 M/M/1)
- ❌ 단일 ARM compute가 chain cap (§6.3 split 무효)
- ❌ host posting/core 수가 chain cap (§4.3 fair≈hw)

---

## 7. 결론 6 — 천장의 *원인* 귀인은 미확증 (이 보고서의 핵심 교훈)

### 7.1 측정으로 확실한 것

1. **chain의 cap은 HW도 EU 수도 host도 in-flight 깊이도 아니다** [측정]. pure 2-EU 1.07M·M0 N=8
   1.53M(HW/drain 여유), fair≈hw(host 무관), depth 2배 평탄(§6.1)이 각각 직접 배제.
2. **chain은 활성 EU 수에 스케일**한다(76K→104K, +37%) [측정]. 2-pod은 `pod%N`으로 2 EU 고정.
3. **chain 천장은 고정-서비스율 큐처럼 거동**한다 [측정, positive] — §6.1의 M/M/1 (throughput 평탄
...
| **chain 416K = 2개 활성 EU의 결합 per-RTT 고정 서비스율** | [귀인] (M/M/1 거동만 [측정]) | M/M/1 거동은 측정됨(§6.1). 그러나 "2 EU 결합 rate"로의 *국소화*는 소거 | ARM compute(split), depth, send | 결정적 실험(독립 multi-ARM 샤딩 / >2 pod로 활성 EU↑) **미실시**; 33% gap을 "reverse+admission+lossless 결합"으로 본 것은 미측정 잔차 |
...
- **multi-EU 1.8M은 측정 하한** — per-op DPU completion 포함. §8의 "~450K RPS 이론 상한"은 이
...
| send 배치 | 효과 작음 | [측정] | send는 병목 아님(§6.3) |

**정직한 한계 — 아직 안 한 결정적 실험:**

1. **chain을 독립 2쌍(4-pod)으로 2 ARM/>2 EU에 샤딩.** throughput이 오르면 "고정 서비스율" 가설
   확정, 안 오르면 반증. 교차 의존을 피해 락 없이 측정. **이 한 실험이 §7.2 첫 두 귀인을 동시에
   판가름한다.** (functional split은 같은 루프를 공유하므로 이 테스트의 대체가 아니다.)
2. **N=8 공유 자원의 positive 국소화** — DPA-side stall-cycle 계측 등. 소거가 아니라 직접 증거로.

---

## 9. comch 메시지층 리팩터 — 비회귀 변경 내역 (2026-06-03)

§3.4·§4.2·§6.3의 리팩터 후 재현 데이터가 모두 회귀 없음을 보였고, 추가로 TX_ACK 압축도 비회귀로
검증됐다. **전 실험 0 fail·slot leak 없음 → comch 메시지층 리팩터는 비회귀(measured).** 천장의
*원인* 귀인은 §7.2 그대로(미측정). 측정 환경 §1.5 동일(BlueField-3, 8KB, dur=10s, fair-pin, EU
affinity ON, `-l 40`). knob은 deploy 시 환경변수로 주입(`DPUMESH_DPA_THREADS`/`DPUMESH_SPLIT_SEND`).
chain은 CSV 미보관 정책(§부록 A)이라 §3.4·§4.2·§6.3·§9.2 표가 1차 기록 — raw 그대로, 누락 없이.

### 9.1 코드 변경 (merge 대상, §부록 B에도 추가)
...
- **host-bound completion `dmesh_dma_completion_msg` 28B→16B**(control path; DPA-side
  ...
  → 처리율 중립(host control send는 chain 병목 아님, §3.4·§6.3과 정합).
...
### 9.2 추가 compaction: TX_ACK 8B + 양쪽 MT 동시 — 검증 [측정]
...
chain 병목이 아님 §6.3). 가치는 dead-필드 제거·wire 축소.

### 9.3 리팩터 후 전체 요약 (raw 기준)

| 구성 | sustainable(≈p99 elbow) | overload achieved | vs N=1 | 0-fail | raw |
|---|---:|---:|---:|---|---|
| N=1 off | ~74K (13.66ms) | 75,201@78K | — | ✅ | §3.4 |
| N=2 off | ~105K (20.58ms) | 104,202 | +41.8% | ✅ | §4.2 |
| N=4 off | ~105K (22.16ms) | 106,994 | +41.8% | ✅ | §4.2 |
| N=2 sends-only | ~105K (25.44ms) | 104,206 | +41.8% | ✅ | §6.3 |
| N=2 rebalanced | ~104K(elbow↓) | 103,139 | +40.4% | ✅ | §6.3 |
| N=2 sends-only + TX_ACK 8B | ~105K (22.11ms) | 104,202 | +41.8% | ✅ | §9.2 |

리팩터 후에도 **§3.2(74K)·§4.1(N=2≈N=4≈104K)·§6.3(split 처리율 중립)이 모두 재현**되고 전 구간
0-fail·slot leak 없음 → **comch 메시지층 리팩터는 비회귀**(measured).
...
| `dpa.c`+`dpa_kernel.c` | multi-EU: `pod%N` 할당, per-EU 2D file-scope admission global, `DPUMESH_DPA_THREADS` knob | §4.1 |
| `dpu_worker.c`+`object.h`+`comch_*` | `DPUMESH_SPLIT_SEND` 0/1/2(SPSC, 3 correctness fix), 기본 0 | §6.3 (진단 토글) |
| `comch_*`/`dpa.c`/`dpu_worker.c`/`ring.c` | hot path `DOCA_LOG_DBG` 제거 | `-l 40` 일관성 |
| `comch_common.h`+`dpa_common.h` | enum 통일 `DMESH_MSG_*`/`DPA_MSG_*`, 0=INVALID, 연속 번호, 죽은 값(EXPORT_DPA_COMP/RX_DATA)+갭 제거 | §9.1 |
| `comch_common.h`+`dpumesh_doca.c`+`comch_client.c` | host-bound `dmesh_dma_completion_msg` 28B→16B + host 디스패치 1B read + `_Static_assert(==16)` | §3.4 (처리율 중립) |
| `comch_server.c`+`comch_client.c`+`dpumesh_doca.c`+`object.h` | vestigial CONSUMER_ID/POD_CONSUMER_ID + `remote_consumer_id` 제거 (consumer 객체 유지) | §9.1 (검증 후) |
| `comch_common.c` | export_desc_len 스택버퍼 경계 가드 | §9.1 안전 |
| `dpumesh_doca.c` | 역방향 RX `length≤slot_size` 가드 + `pos+len` uint32 overflow 수정 + TX_ACK `owner_req_id` 가드 | §9.1 안전 |
| `device/dpa_kernel.c`+`dpa_common.h` | `_Static_assert(DPA_DMA_COPY_MAX≤DPUMESH_SLOT_SIZE)` + cross-ABI `_Static_assert`(dpa_ring_info=72/comch_add_ring_msg=80/comch_msg=84) | §9.1 안전 |
| `comch_common.h`+`comch_server.c` | `dmesh_tx_ack_msg` 12→8B (type 1B + dead `dst_pod_id` 제거) + `_Static_assert(==8)` | §9.2 (처리율 중립) |
