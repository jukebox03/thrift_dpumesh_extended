						...
						uint8_t change_success)
{
	...
	(void)pods_remove_connection(objs, comch_connection);
}
                      ...
                      uint8_t pool_type)
{
	...
	ack.pool_type = pool_type;
	ack.pad[0] = ack.pad[1] = ack.pad[2] = 0;
...
pods_remove_connection(struct objects *objs, struct doca_comch_connection *conn)
{
	...
	if (ring_mmap)
		(void)doca_mmap_destroy(ring_mmap);
	if (remote_mmap)
		(void)doca_mmap_destroy(remote_mmap);
	if (host_rx_mmap)
		(void)doca_mmap_destroy(host_rx_mmap);
