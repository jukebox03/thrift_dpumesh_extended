...
epoll server/client ports by swapping each call for its `dpm_` twin
(`read` → `dpm_read`, …). Header: `thrift/transport/dpm.h` (header-only, built on the
C core `dpumesh.h`).
...
|---|---|---|
| **`dpm_t`** (endpoint) | The bound endpoint. Wraps the whole DPU backend — DOCA device, the RX poller thread, the slot pool, the link to the DPU. Created by `dpm_socket()`. | **one per process**, lives the whole run; **heavy** to create |
| **`dpmconn_t`** (conn) | A handle bound to a **peer** — what an IP:port used to be. Created by `dpm_accept()` (server, an incoming exchange) or `dpm_connect()` (client, outgoing to a pod). | **cheap**; a client conn is **reusable**, a server conn handles one accepted request |
...
- `dpm_connect()` does **no round-trip** — it just binds "this conn talks to pod 11."
...
dpm_t *s = dpm_socket("myapp", 10);
dpmconn_t *c = dpm_connect(s, /*peer*/11);
for (int i = 0; i < N; i++) {
    dpm_write(c, req, len);              // buffer the request
    char resp[8192];
    ssize_t n = dpm_read(c, resp, sizeof resp);   // ships it, then returns the reply
...
| **Setup** | 3-way **handshake** | **none** — `dpm_connect` is local, no round-trip | like UDP/RPC: fire-and-match; peer liveness is *not* checked at connect |
...
| **Blocking** | blocking *or* non-blocking | **non-blocking only** — RX polls; TX busy-spins under local saturation (never blocks on the peer) | to *sleep* until ready, use native epoll on `dpm_event_fd` (below) |
| **`write`** | sends immediately | **buffers**; auto-flushed by the next `dpm_read` (client) / `dpm_close` (server) | socket-style `write`→`read` / `write`→`close`; `dpm_flush` is optional (only the pipelined epoll client needs it) |
...
| **epoll** | the fd itself is pollable | **native epoll on `dpm_event_fd(s)`** — a real fd you register like a listen socket | no `dpm_epoll_*` wrappers; it mixes with real sockets in one epoll set |

**Must-follow rules**
- **Exactly one `dpm_close()` per conn** — every `accept`/`connect` conn, including the
...
- **`dst_pod_id` must be a live, registered pod** `[0,127]`. A dead/unregistered target
  ...
  arrives, so `dpm_read` stays `EAGAIN`. **Apply your own wall-clock timeout.**
...
|---|---|
| `dpm_t *dpm_socket(const char *app_name, int pod_id)` | Endpoint handle, or `NULL` on init failure. `app_name` = service identity (pod registration); `pod_id` = this node's address. |
| `void dpm_destroy(dpm_t *s)` | — (releases all DOCA resources; safe on `NULL`). |
| `int dpm_event_fd(dpm_t *s)` | A real fd that becomes **readable** when an inbound request/response is delivered, for NATIVE epoll/poll/select; `-1` if unavailable. |
| `int dpm_pod_id(dpm_t *s)` / `int dpm_msg_max(dpm_t *s)` | This node's `pod_id` / the max body size (`slot_size`). |
...
| Function | Returns / errno |
|---|---|
| `dpmconn_t *dpm_accept(dpm_t *s)` | New **server** conn holding the next request (body ready), or `NULL`+`EAGAIN` if none pending. **Non-blocking.** (A `NULL` with `errno=ENOMEM` is a rare conn-alloc failure: the request was dropped, its RX slot reclaimed; an accept-until-NULL loop treats it as drained — safe, it just skips one.) |
| `dpmconn_t *dpm_connect(dpm_t *s, int dst_pod_id)` | New **client** conn bound to `dst_pod_id`. No round-trip — a dead/unregistered target is **not** detected here (its response never arrives → `dpm_read` stays `EAGAIN`; apply your own timeout). `NULL`+`ENOMEM` on OOM. |

### Read / write / sendfile / flush
| Function | Returns / errno |
|---|---|
| `ssize_t dpm_read(dpmconn_t *c, void *buf, size_t len)` | `>0` bytes copied from the inbound body; `0` = end of message; `-1` = would-block (client response not in yet, `EAGAIN`) or abandoned (`ECONNRESET`). On a **client** conn the first `read` auto-flushes the buffered request (implicit send). |
| `ssize_t dpm_write(dpmconn_t *c, const void *buf, size_t len)` | **Buffers** outbound body bytes → returns `len`. On a **reused** client conn (its response already read) the first `write` auto-starts a NEW request. `-1` = would exceed `slot_size` (`EMSGSIZE`), or a `write` while a request is still outstanding / a 2nd reply on a server conn (`EINVAL`). Acquiring a TX slot busy-spins under saturation (never fails). |
| `ssize_t dpm_sendfile(dpmconn_t *c, int in_fd, off_t *offset, size_t count)` | Appends ≤`count` bytes from `in_fd` into the body (**capped at `slot_size` → may be SHORT; check the return**); advances `*offset` if non-NULL. Returns bytes appended (`0` = EOF), `-1` on read error / already-outstanding (`EINVAL`) / no slot room (`EMSGSIZE`). |
| `int dpm_flush(dpmconn_t *c)` | **Optional on the happy path** (read/close auto-flush). It is the **explicit ship**: needed only when you ship *before* reading — a pipelined client that fires a window, then harvests via epoll. `0` sent; `-1` with **`EINVAL`** = already sent (not retryable); **`EAGAIN`** = rare `req_id`-table collision, retryable (body kept) but it returns only after a ~2 s internal wait, so a retry is not a fast spin; **`EBADMSG`** = permanent descriptor fault (not retryable; close the conn — unreachable via the façade). |
| `int dpm_close(dpmconn_t *c)` | Ships any buffered-but-unsent message, then frees the conn's slots/pending. `0` ok; **`-1` if that final ship FAILED** (rare `req_id` collision) — the conn is freed either way, so `-1` is *not* retryable. Safe on `NULL`. **ONE-WAY:** a client `write → close` *without a read* is fire-and-forget — the TX slot is freed by the DPU's TX_ACK (no leak) and any reply is silently dropped. For a server reply you must not lose, `dpm_flush()` (retry on `EAGAIN`) before `dpm_close()`. |

**Lifecycles:**
```
server:  c = dpm_accept(s);   dpm_read(c,…)…(EOF);  dpm_write(c,…)…;  dpm_close(c);   // close ships the reply
client:  c = dpm_connect(s,dst);
  RPC (reusable):  for each call: dpm_write(c,…)…;  dpm_read(c,…)…;     // read ships + returns the reply
                   …; dpm_close(c);                                     // once, at the end
  one-way:         dpm_write(c,…)…;  dpm_close(c);                      // fire-and-forget (no read)
```

### Accessors
`int dpm_is_server(dpmconn_t*)`, `int32_t dpm_peer(dpmconn_t*)` (the conn's peer pod),
`void dpm_set_data(dpmconn_t*, void*)` / `void *dpm_get_data(dpmconn_t*)` (mirrors
...
dpm_t *s = dpm_socket("echo", /*pod_id*/11);
for (;;) {
    dpmconn_t *c = dpm_accept(s);          // non-blocking
    if (!c) { /* errno==EAGAIN: idle — sched_yield() or do other work */ continue; }

    char buf[8192]; ssize_t n, off = 0;
    while ((n = dpm_read(c, buf + off, sizeof buf - off)) > 0) off += n;   // whole request
    dpm_write(c, buf, off);                 // echo it back (<= 8 KB)
    dpm_close(c);                            // ships the reply + frees the RX slot
}
```
*(For throughput, run this loop on N threads — the endpoint is shared and thread-safe.)*

> **Delivery:** `dpm_close` auto-ships the reply but can only *report* a ship failure
> via its return (`-1`), not retry it. If losing a reply is unacceptable, ship first:
> `while (dpm_flush(c) < 0 && errno == EAGAIN) sched_yield();` then `dpm_close(c)`.
...
dpm_t *s = dpm_socket("client", /*pod_id*/10);
dpmconn_t *c = dpm_connect(s, /*dst_pod_id*/11);

for (int i = 0; i < N; i++) {
    dpm_write(c, req[i], req_len[i]);            // buffer; the read below ships it
    struct timespec t0; clock_gettime(CLOCK_MONOTONIC, &t0);
    char resp[8192]; ssize_t n;
    for (;;) {
        n = dpm_read(c, resp, sizeof resp);
...
reused — close it; `dpm_connect` is free.)
```c
for (int i = 0; i < N; i++) {
    dpmconn_t *c = dpm_connect(s, /*dst_pod_id*/11);
    dpm_write(c, msg[i], msg_len[i]);
    dpm_close(c);                           // fire-and-forget
}
```
...
`dpm_event_fd(s)`. Two things matter: **(1) flush explicitly** — you sleep on
`epoll_wait` *before* reading, so you can't rely on read-triggered implicit send;
**(2)** `dpm_event_fd` is **per-endpoint**, so a readiness event doesn't say *which*
conn — tag each with `dpm_set_data` and scan your in-flight conns on wakeup.
```c
#include <thrift/transport/dpm.h>
#include <sys/epoll.h>
#include <time.h>
...
for (int i = 0; i < W; i++) {                        // ── launch the window ──
    dpmconn_t *c = dpm_connect(s, /*dst_pod_id*/11);
    dpm_set_data(c, (void *)(intptr_t)i);            // tag it (e.g. the request index)
    dpm_write(c, req[i], req_len[i]);
    while (dpm_flush(c) < 0 && errno == EAGAIN) sched_yield();   // ship NOW (we'll sleep on epoll)
...
while (pending > 0) {                                 // ── harvest via epoll ──
    ...
    for (int e = 0; e < nfds; e++) {
        ...
        for (int i = 0; i < W; i++) {                // per-endpoint fd → scan in-flight conns
            ...
            if (!c) continue;
            char resp[8192];
            ssize_t n = dpm_read(c, resp, sizeof resp);
            if (n < 0 && errno == EAGAIN) continue;  // this one's reply not in yet
            int idx = (int)(intptr_t)dpm_get_data(c);
            handle_response(idx, resp, n);           // n>=0 = reply (0 = empty); n<0 = ECONNRESET
            dpm_close(c); inflight[i] = NULL; pending--;
        }
    }
    for (int i = 0; i < W; i++)                       // abandon dead-pod conns (EAGAIN never clears)
        if (inflight[i] && time(NULL) - launched[i] >= 5) {
            dpm_close(inflight[i]); inflight[i] = NULL; pending--;   // count as a timeout failure
        }
}
```
> A reply is a whole ≤8 KB message, so one `dpm_read` returns it in full. For a large
...
epoll**; `dpm_event_fd(s)` is the "listen socket".
```c
dpm_t *s = dpm_socket("echo", 11);
int dfd  = dpm_event_fd(s);                              // the DPUmesh readiness fd
...
for (;;) {
    ...
    for (int n = 0; n < nfds; n++) {
        ...
        dpmconn_t *c;
        while ((c = dpm_accept(s)) != NULL) {            // accept every queued request
            char b[8192]; ssize_t off = 0, r;
            while ((r = dpm_read(c, b + off, sizeof b - off)) > 0) off += r;   // full request
            if (off > 0) dpm_write(c, b, off);           // echo it back
            dpm_close(c);                                 // ships it (implicit send)
        }
    }
}
```
> The diff from an ordinary TCP epoll echo server is **only** the `dpm_` prefix on
> `socket/accept/read/write/close` (no extra send — `close` auto-flushes the reply) and
> using `dpm_event_fd(s)` as the listen fd. The epoll loop is unchanged.
...
with `dpm_write`/`dpm_sendfile`, and `dpm_close()` ships it:
```c
dpm_t *s = dpm_socket("httpd", pod_id);
for (;;) {
    dpmconn_t *c = dpm_accept(s);
    if (!c) { sched_yield(); continue; }

    char req[8192]; ssize_t off = 0, n;
    while ((n = dpm_read(c, req + off, sizeof req - off)) > 0) off += n;   // full request header

    if (parse_request(req, off) < 0) {
        dpm_write(c, ERR_400, strlen(ERR_400));
    } else {
        int fd = open_file(url);
        if (fd < 0) dpm_write(c, ERR_404, strlen(ERR_404));
        else if (flen + 256 > (long)dpm_msg_max(s))      // header + body won't fit one ≤8KB msg
            dpm_write(c, ERR_500, strlen(ERR_500));      // (or chunk across round-trips)
        else {
            char hdr[256];
            int hl = snprintf(hdr, sizeof hdr, "HTTP/1.0 200 OK\r\nContent-Length: %ld\r\n\r\n", flen);
            dpm_write(c, hdr, hl);
            ssize_t w = dpm_sendfile(c, fd, NULL, flen);   // capped at slot_size → may be SHORT
            close(fd);
            if (w < (ssize_t)flen) { /* body didn't fit one message — chunk or send an error */ }
        }
    }
    dpm_close(c);   // ships the one atomic response (implicit send)
}
```
